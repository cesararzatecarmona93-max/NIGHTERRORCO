...
class GWorkspaceBridge:
    ...
    def __init__(self):
        ...
        client_data_str = os.getenv("GOOGLE_CLIENT_SECRET_JSON", "{}")
        client_data = json.loads(client_data_str) if client_data_str else {}
        self.client_creds = create_secret_from_dict(client_data) if client_data else None

        user_data_str = os.getenv("GOOGLE_USER_TOKEN_JSON", "{}")
        self.user_creds = json.loads(user_data_str) if user_data_str else {}
...
async def process_webhook(
    ...
    """
    # Validación de API Key (Seguridad básica)
    expected_key = os.getenv("L2C_INTERNAL_API_KEY")
