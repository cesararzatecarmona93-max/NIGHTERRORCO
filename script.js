const AGENTS = {
    CONTEXT_ENGINEER: {
        title: "Agente de Ingeniería de Contexto",
        role: "Apex-Level Cognitive Architect",
        description: "Meta-Prompt God Mode diseñado para transmutar instrucciones crudas en Llaves Maestras.",
...
# Output: A strategic pricing blueprint and a Launch Roadmap.`
    ...
    LEGAL_SENTINEL: {
        ...
        description: "Analizar contratos y generar reporte de 'Semáforo de Riesgo'.",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const terminalBadge = document.querySelector('.terminal-header .badge');
    ...
    Object.keys(AGENTS).forEach(key => {
        ...
        `;
        item.addEventListener('click', () => selectAgent(key, item));
    ...
    if (firstKey) {
        selectAgent(firstKey, sidebar.querySelector('.agent-item'));
    }
    ...
    function selectAgent(key, element) {
        ...
        const agent = AGENTS[key];
        terminalTitle.textContent = agent.title;
        terminalBadge.textContent = "READY";

        // Typewriter effect for System Prompt
        typewriter(agent.systemPrompt, terminal);
    }
