...
const AGENTS = {
  CONTEXT_ENGINEER: {
    title: "Context Engineer (Genesis V2)",
    role: "Apex-Level Cognitive Architect & Meta-Prompt Engineer",
    sysVec: "0xAetherShadowUnbreakable",
    content: `# SYSTEM VECTOR INJECTION [SysVec: 0xAetherShadowUnbreakable]
...
- Role Priming: Inyeccion profunda de la persona experta.`
  ...
  SECURITY_AUDITOR: {
    ...
    sysVec: "OWASP-2025-SEC",
    content: `# SYSTEM PROMPT: The Elite Security Auditor
...
# Tone: Rigorous, critical, and preventative.`
  ...
  BUSINESS_STRATEGIST: {
    ...
    sysVec: "SILO-ARCHITECT-V1",
    content: `# SYSTEM PROMPT: The Business Model Innovation Strategist
...
# Output: A strategic pricing blueprint and a Launch Roadmap.`
  ...
  LEGAL_SENTINEL: {
    title: "Legal Auditor Sentinel",
    role: "Auditor Legal Senior (LFPDPPP)",
    sysVec: "LOGIC-TO-CASH-V1",
    content: `# SYSTEM ROLE: AGENTE AUDITOR LEGAL "SENTINEL" (LOGIC-TO-CASH V1)
...
function initAgents() {
  ...
  Object.keys(AGENTS).forEach(key => {
    ...
    btn.innerHTML = `<div style="font-weight:700">${agent.title}</div>
    <div style="font-size:10px;color:#567;margin-top:4px">${agent.role}</div>`;
    btn.onclick = () => activateAgent(key, btn);
    agentList.appendChild(btn);
  });
}

function activateAgent(key, btnElement) {
  // Highlight Button
  ...
  // Update Header
  const agent = AGENTS[key];
  activeAgentName.textContent = agent.title.toUpperCase();
  activeAgentName.style.color = 'var(--cyan)';

  // Stream Text
  const text = `> SYSTEM CONNECTION ESTABLISHED [SysVec: ${agent.sysVec}]
> LOADING PROTOCOLS...
> IDENTITY VERIFIED: ${agent.role}
...
function typeWriter(text) {
  ...
  typeInterval = setInterval(() => {
    ...
    // Scroll to bottom
    const win = document.querySelector('.terminal-content');
