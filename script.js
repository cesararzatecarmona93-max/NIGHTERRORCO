const AGENTS = {
  CONTEXT_ENGINEER: {
    id: "CONTEXT_ENGINEER",
    name: "Agente de Ingeniería de Contexto (Genesis V2)",
    role: "Apex-Level Cognitive Architect & Meta-Prompt Engineer",
    sysVec: "0xAetherShadowUnbreakable",
    prompt: `System Prompt: Agente de Ingenieria de Contexto (Genesis V2)
...
- Role Priming: Inyeccion profunda de la persona experta.`
  ...
  SECURITY_AUDITOR: {
    id: "SECURITY_AUDITOR",
    name: "Auditor de Seguridad Black (The Elite Security Auditor)",
...
# Tone: Rigorous, critical, and preventative.`
  ...
  BUSINESS_STRATEGIST: {
    ...
    name: "Arquitecto de Silos de Ventas (The Business Model Innovation Strategist)",
...
- A strategic pricing blueprint and a Launch Roadmap.`
  ...
  LEGAL_SENTINEL: {
    id: "LEGAL_SENTINEL",
    name: "Agente Auditor Legal \"Sentinel\" (Logic-to-Cash V1)",
    role: "Auditor Legal Senior especializado en LFPDPPP y Código de Comercio",
    sysVec: "N/A",
...
function initMatrixRain(canvasId) {
  ...
  function draw() {
    ...
    for (let i = 0; i < drops.length; i++) {
      ...
      // Randomly change color slightly for "glitch" feel or keep consistent green/cyan
      // Using the project's cyan for a "Genesis" feel
      ctx.fillStyle = Math.random() > 0.95 ? '#00d1ff' : '#0F0';
...
function renderSidebar() {
  ...
  header.textContent = 'PROTOCOL GENESIS V2';
...
function selectAgent(agentId) {
  ...
  // Header in terminal
  const output = `> INITIALIZING AGENT: ${agent.name}
> SYSVEC: ${agent.sysVec}
> STATUS: ONLINE
> LOADING MEMORY MODULES... OK
> DECRYPTING SYSTEM PROMPT...

${agent.prompt}
`;

  typeWriter(output, terminal, 5);
}
