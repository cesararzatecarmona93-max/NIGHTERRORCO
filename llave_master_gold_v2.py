...
class GovernanceSentinel(BaseAgent):
    ...
    async def process(self, context: InstructionalSegment) -> AgentResponse:
        ...
        triggers = ["cambio pequeño", "rápido", "solo esto más", "agrégalo", "sin costo", "detalle"]
...
class NexusEngine:
    ...
    async def semantic_router(self, segment: InstructionalSegment) -> AgentResponse:
        ...
        if any(x in content for x in ["legacy", "código viejo", "php", "python 2.7", "migrar"]):
            ...
            # Fallback seguro
            audit_hash = self.auditor.log_event("SYSTEM_ROUTER", "ROUTING_ERROR", "Solicitud no reconocida")
            return AgentResponse(
    ...
    async def execute_request(self, raw_input: str) -> str:
        """
        Punto de entrada principal. Ejecuta el ciclo completo de procesamiento.
        """
        # 1. Purificación S2A
        ...
        # Formateo técnico para consola (0x0_MIN_EXEC_ENGINE protocol)
        # La salida es estrictamente la información técnica procesada.
        return response.content

# =============================================================================
# 5. QUALITY SUITE (PYTEST INTEGRADO)
# =============================================================================

import pytest

def test_sovereign_vocabulary_validator():
    """Test unitario para la restricción de vocabulario."""
    with pytest.raises(ValidationError) as exc_info:
        AgentResponse(
            agent_id="TEST",
            content="Te ofrezco un servicio barato con un gran descuento",
            audit_hash="fake_hash"
        )
    # Extraer el mensaje real y verificar las palabras prohibidas detectadas
    err_msg = str(exc_info.value)
    assert "servicio" in err_msg
    assert "barato" in err_msg
    assert "descuento" in err_msg

def test_sovereign_vocabulary_valid():
    """Test unitario para vocabulario permitido."""
    response = AgentResponse(
        agent_id="TEST",
        content="Esta Inversión representa un Activo de alta Certeza.",
        audit_hash="fake_hash"
    )
    assert response.content == "Esta Inversión representa un Activo de alta Certeza."

@pytest.mark.asyncio
async def test_nexus_engine_routing_lazarus():
    """Test de enrutamiento semántico hacia Lazarus."""
    engine = NexusEngine()
    result = await engine.execute_request("Por favor migrar este sistema PHP a Python 3.")
    assert "FROM python" in result
    assert "DOCKERFILE" in result

@pytest.mark.asyncio
async def test_nexus_engine_routing_sentinel():
    """Test de enrutamiento semántico hacia Sentinel (Scope Creep)."""
    engine = NexusEngine()
    result = await engine.execute_request("Añade este cambio pequeño sin costo extra.")
    assert "NOTIFICACIÓN FORMAL DE CONTROL DE CAMBIOS" in result
    assert "$150,000 USD" in result

@pytest.mark.asyncio
async def test_nexus_engine_routing_finance():
    """Test de enrutamiento semántico hacia PoT Engine."""
    engine = NexusEngine()
    result = await engine.execute_request('{"revenue": 100000, "costs": 40000, "tax_rate": 0.3}')
    assert "REPORTE FINANCIERO BLINDADO" in result
    assert "Utilidad Neta:     $42,000.00" in result

def test_merkle_tree_integrity():
    """Test del Auditor Forense (Inmutabilidad)."""
    auditor = ForensicAuditor()
    hash1 = auditor.log_event("TEST", "ACTION1", "Result 1")
    hash2 = auditor.log_event("TEST", "ACTION2", "Result 2")

    assert auditor.verify_integrity() is True
    assert auditor.get_root_hash() != ""
    assert hash1 in auditor.leaves

    # Simulación de ataque: Modificar un log
    auditor.leaves[0] = "corrupted_hash"
    assert auditor.verify_integrity() is False


# =============================================================================
# 6. EJECUCIÓN CLI (DEMOSTRACIÓN)
# =============================================================================
...
async def main():
    ...
    """
    logger.info("=== INICIANDO NEXUS ENGINE (LLAVE MÁSTER GOLD V2) ===")
    engine = NexusEngine()

    # Simulación de Casos de Uso
    casos = [
        "Por favor, revisa este código viejo en PHP que necesitamos migrar urgentemente.",
        "Cliente: Oye, agrégalo al proyecto original, es un detalle rápido sin costo.",
        'Calcula esto: {"revenue": 500000, "costs": 150000, "tax_rate": 0.35}'
    ]

    for idx, caso in enumerate(casos, 1):
        logger.info(f"\n--- CASO {idx} ---")
        logger.info(f"INPUT RAW: {caso}")
        try:
            resultado = await engine.execute_request(caso)
            print(f"\n[OUTPUT TÉCNICO]\n{resultado}\n")
        except Exception as e:
            logger.error(f"Error procesando caso {idx}: {e}")
