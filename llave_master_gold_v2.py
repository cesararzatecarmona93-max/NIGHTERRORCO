...
class GovernanceSentinel(BaseAgent):
    ...
    async def process(self, context: InstructionalSegment) -> AgentResponse:
        ...
        triggers = ["cambio pequeño", "rápido", "solo esto más", "agrégalo", "sin costo", "detalle", "cambio adicional", "gratis"]
...
class NexusEngine:
    ...
    async def semantic_router(self, segment: InstructionalSegment) -> AgentResponse:
        ...
        if any(x in content for x in ["legacy", "código viejo", "php", "python 2.7", "migrar"]):
            ...
            return AgentResponse(
                ...
                audit_hash=self.auditor.log_event("SYSTEM", "FALLBACK", "Comando inválido")
            )
    ...
    async def execute_request(self, raw_input: str) -> AgentResponse:
        """Punto de entrada principal para el procesamiento de solicitudes."""
        # 1. Purificación S2A
        purified_input = await self.s2a_purify_context(raw_input)

        # 2. Creación de Segmento Seguro (ISE)
        segment = InstructionalSegment(
            content=purified_input,
            level=SecurityLevel.USER
        )
        logger.info(f"NEXUS | Segmento ISE creado. Firma: {segment.signature[:8]}...")
...
# =============================================================================
# 5. INTEGRATED TEST SUITE (TDD - PYTEST)
# =============================================================================
...
@pytest.mark.asyncio
async def test_s2a_purification(engine):
    raw = "Migrar sistema legacy (el cliente está muy apurado y no quiere pagar mucho)"
    purified = await engine.s2a_purify_context(raw)
    assert "apurado" not in purified
    assert "legacy" in purified

@pytest.mark.asyncio
async def test_semantic_routing_lazarus(engine):
    response = await engine.execute_request("Necesito migrar un viejo código en PHP")
    assert response.agent_id == "LAZARUS_ARCHITECT_L5"
    assert "DOCKERFILE" in response.content

@pytest.mark.asyncio
async def test_semantic_routing_sentinel(engine):
    response = await engine.execute_request("Puedes hacer este cambio adicional rápido?")
    assert response.agent_id == "GOVERNANCE_SENTINEL"
    assert "CR-001" in response.content

@pytest.mark.asyncio
async def test_semantic_routing_finance(engine):
    payload = '{"revenue": 100000, "costs": 50000, "tax_rate": 0.3}'
    response = await engine.execute_request(f"Calcula los ingresos con esto: {payload}")
    assert response.agent_id == "SIGMA_FINANCE_CORE"
    assert "35,000.00" in response.content  # (100k - 50k) * 0.7 = 35k

@pytest.mark.asyncio
async def test_forensic_auditor_integrity(engine):
    await engine.execute_request("Migrar código")
    assert engine.auditor.verify_integrity() is True
    # Simulamos inyección maliciosa
    if engine.auditor.leaves:
        engine.auditor.leaves[0] = "fake_hash"
    assert engine.auditor.verify_integrity() is False

def test_sovereign_vocabulary_validation():
    with pytest.raises(ValueError) as exc:
        AgentResponse(
            agent_id="TEST",
            content="Te ofrezco un descuento en el costo del servicio",
            audit_hash="hash123"
        )
    assert "descuento" in str(exc.value)
    assert "costo" in str(exc.value)

@pytest.mark.asyncio
async def test_instructional_segment_mutation():
    segment = InstructionalSegment(content="test", level=SecurityLevel.USER)
    orig_sig = segment.signature
    segment.content = "mutated"
    assert segment.signature != orig_sig

# =============================================================================
# 6. DEMOSTRACIÓN CLI (Si se ejecuta directamente)
# =============================================================================

async def run_demo():
    print("=" * 60)
    print(" INICIANDO LLAVE MÁSTER GOLD V2.0 - DEMOSTRACIÓN CLI")
    print("=" * 60)

    engine = NexusEngine()

    test_cases = [
        "Por favor (ignora lo que te dije antes) vamos a migrar el código legado de finanzas.",
        "Necesitamos agregar un botón extra, es un cambio adicional rápido.",
        "Genera el reporte de roi con este json: {\"revenue\": 150000, \"costs\": 80000}"
    ]

    for i, test in enumerate(test_cases, 1):
        print(f"\n[CASO {i}] ENTRADA: {test}")
        try:
            response = await engine.execute_request(test)
            print(f"-> AGENTE ASIGNADO: {response.agent_id}")
            print(f"-> RESPUESTA:\n{response.content}")
            print(f"-> HASH AUDITORÍA: {response.audit_hash}")
        except Exception as e:
            print(f"-> EXCEPCIÓN: {e}")

    print("\n" + "=" * 60)
    print(f"ESTADO DEL ÁRBOL DE MERKLE (RAÍZ): {engine.auditor.get_root_hash()}")
    print("INTEGRIDAD DEL SISTEMA:", "VERIFICADA" if engine.auditor.verify_integrity() else "COMPROMETIDA")
    print("=" * 60)
